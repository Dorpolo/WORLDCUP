...
server <- function(input, output, session) {
  ...
  # Reactive Data frame for the users league
  polo <- reactive({ 
  ...
  # Results <> Users <> Network Graph - Datat Preperation 
  {
  ...
  ### Cup Table Reactive data frame
  chava_the_data_img <- reactive({ready_data_cup %>% filter(Stage %in% input$cup_round)  %>% 
