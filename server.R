...
shinyServer(function(input, output) {
  ...
  output$league_table <- DT::renderDataTable({
    ...
    all_df_list$userID$fixtures <- fixtures
    ...
    DT::datatable(data = user_results_validation %>% 
                  ...
                  options = list(pageLength = N_users,
                                 initComplete = JS(
                                   ...
                                   "$(this.api().table().header()).css({'background-color': '#F9E79F', 'color': '#00384A'});",
                                   "}"),
                                 searching = FALSE,
  ...
  output$pre_game_I <- renderPlot({
    ggplot(next_game_tbl(), aes(y = proportion, x = "", fill = team_direction)) +
      ...
      theme(
        ...
        panel.background = element_rect(fill = "#2b3e50", colour = "#2b3e50"),
  ...
  output$title <- renderUI({
    ...
    HTML(paste0('<div style = "background-color: #F70443; width: 100%; height: 56px; border-radius: 4px;">
                <center> <font size="4"> <font color="#F8F9F9"> Live Score : <br>',text,
                '</div>'))
  })
  ...
  # Additonal column with image HTML url's
