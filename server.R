...
## Points per game per user
{
  ## Inage HTML viewer
  {
    img_url <- function(x) { sprintf('<img src="%s" height="30" width="30" />',x) }
  }
...
## Data Frame Adjustments (Might be for diffrrent reasons)

# I Did it for be able to use Nick Name instead of the Full Name
resultes <- resultes %>% left_join(User_ID %>% select(Full.Name,User_Nick),by = c('User.Name'='Full.Name'))
User_ID <- User_ID %>% mutate(Img = img_url(Picture))
...
current_cup_stage_name <- ifelse(current_game>48,
                                 fixtures$Cup_Stage[which(fixtures$GameID == current_game)],
                                 fixtures$Cup_Stage[1])
